...
class Reshape_feature(nn.Module):
    def __init__(self, dim, dim_out, norm_layer=nn.LayerNorm, scale_factor=4):
        super().__init__()
...
class VGGBlock(nn.Module):
    def __init__(self, in_channels, middle_channels, out_channels):
        ...
        self.relu = nn.ReLU(inplace=True) # True: Replace the original data
...
class conv_block(nn.Module):
    def __init__(self, in_ch, out_ch):
        ...
        self.conv11 = nn.Conv2d(in_ch, out_ch, 1)
...
class CSWinTransformer(nn.Module):
    ...
    """
    def __init__(self, img_size=224, patch_size=16, in_chans=1, num_classes=1, embed_dim=96, depth=[2,2,6,2], split_size = [3,5,7],
        ...
        self.stage1 = nn.ModuleList([
            CSWinBlock(
                dim=curr_dim, num_heads=heads[0], reso=img_size, mlp_ratio=mlp_ratio,
        ...
        temp_stage3.extend(
            [CSWinBlock(
                dim=curr_dim, num_heads=heads[2], reso=img_size//4, mlp_ratio=mlp_ratio,
        ...
        self.stage4 = nn.ModuleList(
            [CSWinBlock(
                dim=curr_dim, num_heads=heads[3], reso=img_size//8, mlp_ratio=mlp_ratio,
                qkv_bias=qkv_bias, qk_scale=qk_scale, split_size=split_size[-1],
                drop=drop_rate, attn_drop=attn_drop_rate,
        ...
        #################
        nb_filter = [96, 192, 384, 768, 1536]

        in_dim = [96, 192, 384, 768]
        out_dim = [64, 128, 256, 512]
        self.reshape1 = Reshape_feature(in_dim[0], out_dim[0], scale_factor=4)
        self.reshape2 = Reshape_feature(in_dim[1], out_dim[1], scale_factor=4)
        self.reshape3 = Reshape_feature(in_dim[2], out_dim[2], scale_factor=4)
        self.reshape4 = Reshape_feature(in_dim[3], out_dim[3], scale_factor=4)
        ##################
        ...
        self.conv1 = conv_block(nb_filter[0], nb_filter[0])
        self.conv2 = conv_block(nb_filter[1], nb_filter[1])
        self.conv3 = conv_block(nb_filter[2], nb_filter[2])
        self.conv4 = conv_block(nb_filter[3], nb_filter[3])
    ...
    def forward_features(self, x):
        ...
        u[0] = self.reshape1(u[0]) #torch.Size([8, 64, 224, 224])
        u[1] = self.reshape2(u[1]) #torch.Size([8, 128, 112, 112]) 
        u[2] = self.reshape3(u[2]) #torch.Size([8, 256, 56, 56]) 
        u[3] = self.reshape4(u[3]) #torch.Size([8, 512, 28, 28])
        x4_0 = self.conv4_0(self.pool(u[3])) # torch.Size([8, 1024, 14, 14])
        ...
        x3_1 = self.conv3_1(torch.cat([u[3], self.up(x4_0)], 1))  # torch.Size([8, 512, 28, 28]) 
        x2_2 = self.conv2_2(torch.cat([u[2], self.up(x3_1)], 1)) # torch.Size([8, 256, 56, 56]) 
        x1_3 = self.conv1_3(torch.cat([u[1], self.up(x2_2)], 1)) # torch.Size([8, 128, 112, 112]) 
        x0_4 = self.conv0_4(torch.cat([u[0], self.up(x1_3)], 1)) # torch.Size([8, 64, 224, 224])


        # print(x0_4.shape, x1_3.shape, x2_2.shape, x3_1.shape, x4_0.shape)
